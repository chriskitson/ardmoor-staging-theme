...
(function () { // Add this
  ...
  function buildLabels(data) {
    // Build Sale label
    var saleLabelHtml = '';
    var isOnSale = data.tags.includes('Sale') || data.tags.includes('sale') ? true : false;
    if (isOnSale) {
      var saveAmount = '';
      if (boostPFSConfig.custom.discount_mode) {
    ...
    if (data.tags && data.tags.length > 0) {
      ...
      for (var i = 0; i < data.tags.length; i++) {
        ...
        if (tag.indexOf('New') !== -1) {
          newLabelHtml = '<span class="product-button new-product">New</span>';
        }
      ...
      tagLabelHtmls = newLabelHtml + tagLabelHtml1 + tagLabelHtml2;
    }
  ...
  function buildPrice(data) {
    var onSaleClass = onSale ? 'price--highlight' : '';
    ...
    if (data.price_min) {
      priceHtml += '<span class="price '+ onSaleClass +'">'+ priceAmount +'</span>';
    }
